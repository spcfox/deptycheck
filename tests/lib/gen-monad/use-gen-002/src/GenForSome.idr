...
genSo : (b : Bool) -> Gen MaybeEmpty $ So b
genSo True  = pure Oh
genSo False = empty

genAllQ : ((x : a) -> Gen MaybeEmpty (p x)) -> (list : List a) -> Gen MaybeEmpty $ All p list
genAllQ f []      = [| []                  |]
genAllQ f (x::xs) = [| f x :: genAllQ f xs |]

genAnyQ : ((x : a) -> Gen MaybeEmpty (p x)) -> (list : List a) -> Gen MaybeEmpty $ Any p list
genAnyQ f []      = empty
...
genUniqueName : (existing : List Name) -> Gen MaybeEmpty (new : Name ** All (\ex => So $ ex /= new) existing)
genUniqueName existing = do
  new <- genName
  all <- genAllQ (\ex => genSo (ex /= new)) existing
  pure (new ** all)

genExisting : Eq a => (existing : List a) -> Gen MaybeEmpty (res : a ** Any (\ex => So $ ex == res) existing)
genExisting [] = empty
...
mutual
  ...
  export
  genStmt__ : Fuel -> Gen MaybeEmpty (definedPre ** definedPost ** Stmts definedPre definedPost)
  ...
  export
  genStmtP_ : Fuel -> (definedPre : _) -> Gen MaybeEmpty (definedPost ** Stmts definedPre definedPost)
  ...
  export
  genStmt_P : Fuel -> (definedPost : _) -> Gen MaybeEmpty (definedPre ** Stmts definedPre definedPost)
  ...
  export
  genStmtPP : Fuel -> (definedPre : _) -> (definedPost : _) -> Gen MaybeEmpty $ Stmts definedPre definedPost
