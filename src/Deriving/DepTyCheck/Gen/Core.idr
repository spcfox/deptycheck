...
ConstructorDerivator => DerivatorCore where
  canonicBody sig n = do
    ...
    fuelArg <- genPrettyName "^fuel_arg^" -- I'm using a name containing chars that cannot be present in the code parsed from the Idris frontend
  ...
  where
    ...
    namesWrapper s = UN $ Basic "inter^<\{show s}>"

    fuelDecisionExpr : (fuelArg : Name) -> List (Con, Recursiveness) -> TTImp
    fuelDecisionExpr fuelAr consRecs = do
      ...
      let True = isJust $ find (isRec . snd) consRecs
        | False =>
            ...
            callOneOf "\{logPosition sig} (non-recursive)".label (consRecs <&> callConsGen (var fuelAr) . fst)
      ...
      iCase .| var fuelAr .| var `{Data.Fuel.Fuel} .|
        ...
        , do -- if fuel is `More`, spend one fuel and call all constructors on the rest
          ...
          let selectFuel = \r => var $ if isDirectlyRec r then subFuelArg else fuelAr
          let weight = \r => if isRec r then var `{Deriving.DepTyCheck.Util.Reflection.leftDepth} .$ var subFuelArg else liftWeight1
