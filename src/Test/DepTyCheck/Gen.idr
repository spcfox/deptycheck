...
-- TODO: composition is not total
IsNonEmpty g = So $ isNonEmpty g
...
0 strengthenNonEmpty : {g : Gen em a} -> IsJust (strengthen g) =>
                     IsNonEmpty $ fromJust $ Gen.strengthen g
  ...
  where
    ...
    helper (w, x) with (f x) proof 0 prf
      helper (w, x) | Just y = do
        ...
        let reduceFromJust : y === fromJust (f x) = rewrite prf in Refl
        rewrite reduceFromJust
        h x yIsJust
